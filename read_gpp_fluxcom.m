% Get 1982-2013 GPP data from FluxCom and format for comparison to other data
...
%% Units for final GPP data (original: g C m-2 day-1)
...
info = ncinfo('GPP.RF.CRUNCEPv6.monthly.1980.nc');
lat = ncread('GPP.RF.CRUNCEPv6.monthly.1980.nc', 'lat');
lon = ncread('GPP.RF.CRUNCEPv6.monthly.1980.nc', 'lon');
...
for i = 1:length(years)
    ...
    fn = ['GPP.RF.CRUNCEPv6.monthly.',num2str(years(i)),'.nc'];
    gpp = ncread(fn, 'GPP');
    gpp(gpp == -9999) = NaN;
    gpp = 0.001 * permute(gpp, [2 1 3]); % from gC m-2 day-1 --> kgC m-2 day-1
